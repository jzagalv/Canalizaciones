...
class ResultsTab(QWidget):
    ...
    def set_results(self, project: Project, solutions: Dict[str, Dict], warnings: List[str]) -> None:
        ...
        for edge_id, sol in (solutions or {}).items():
            ...
            self.tbl.setItem(r, 2, QTableWidgetItem(str(sol.get('proposal', ''))))
            self.tbl.setItem(r, 3, QTableWidgetItem(str(sol.get('fill', ''))))
            self.tbl.setItem(r, 4, QTableWidgetItem(str(sol.get('status', ''))))
            self.tbl.setItem(r, 5, QTableWidgetItem(str(sol.get('notes', ''))))
